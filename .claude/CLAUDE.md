...
- Обращение `[assistant] address` (сэр / мисс / любое слово): в системный промпт LLM и, если оно не «сэр» и `tts.backend = "http"`, в отклики — `phrases.rs` синтезирует их на голосовом сервере в `%APPDATA%\com.priler.jarvis\phrases\` (FNV-хеш текста) и `voices::play` берёт их вместо записанных. Установщик передаёт `configure.ps1 -Address sir|miss`; `configure.ps1` пишет отчёт без ключей в `configure.log`.
