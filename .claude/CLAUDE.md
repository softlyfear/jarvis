...
3. Если команда не найдена или действие не нашло объект (`ActionError::NotFound`), фраза уходит в LLM: `jarvis-core/src/llm.rs`. Провайдер по умолчанию — Gemini; `models = ["auto"]` (`llm/models.rs`) берёт список моделей у API и ранжирует их. Необязательный платный ключ Kilo/OpenRouter (блок `name = "paid"`, `PAID_MODELS`) идёт первым; 402 (нет денег) усыпляет ключ на час. Последними без ключа — бесплатные модели Kilo (`KILO_FREE_MODELS`, порядок по бенчмарку, `[llm] free_fallback`, добавляются в `assistant_config::parse` и в старые конфиги); `free_only` оставляет только их. Провайдер, упавший по сети (таймаут), 2 минуты пропускается. Ключи чередуются; модель вызывает те же действия как tools (`llm/tools.rs`).
