...
- Пока Джарвис говорит (`audio::hold_microphone`, по длительности звука; SAPI — на время вызова), слушатель пропускает микрофон: иначе он распознаёт свои ответы как команды. Исключение — ответ клоном голоса: пока он звучит, `app::wait_for_wake_word` слушает только слово активации, и «Джарвис» обрывает речь (`audio::stop_speaking`, `[tts] barge_in`); ответ, где он сам называет себя, не прерывается.
